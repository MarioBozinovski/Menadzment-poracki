...
namespace Csharp_Project
{
    public partial class FORM_MANAGE_USER : Form
    {
        ...
        private void BTN_DELETE_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Dali sakate da go izbrisite ovoj korisnik", "Izbrisi korisnik", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    user.deleteUser(Convert.ToInt32(TB_ID.Text));
                    MessageBox.Show("Korisnikot e uspesno izbrisan", "Izbrisi korisnik", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    DGV_USERS.DataSource = user.getUsers();
                }
            }
            else
            {
                MessageBox.Show("Selektiraj korisnik za brisenje");
            }
        ...
        private void BTN_UPDATE_Click(object sender, EventArgs e)
        {
            if (TB_ID.Text != string.Empty)
            {
                ...
                MessageBox.Show("Korisnikot e uspesno azuriran", "Azuriraj korisnik", MessageBoxButtons.OK, MessageBoxIcon.Information);
                DGV_USERS.DataSource = user.getUsers();
            }

            else
            {
                MessageBox.Show("Izberi korisnik za azuriranje");
            }
        }
