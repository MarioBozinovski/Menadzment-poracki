...
namespace Csharp_Project
{
    public partial class FORM_MANAGE_PRODUCT : Form
    {
        ...
        {
            if (MessageBox.Show("Dali navistina sakate da go izbrisete ovoj proizvod", "Izbrisi proizvod", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                product.deleteProducts(Convert.ToInt32(DGV_PRODUCTS.CurrentRow.Cells[0].Value));
                DGV_PRODUCTS.DataSource = product.getProducts();
                MessageBox.Show("Proizvodot e uspesno izbrisan", "Izbrisi proizvod");
            }
        }
